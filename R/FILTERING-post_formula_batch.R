...
#' @export
post_formula_batch <- function(formulas, data_sources = NULL, 
                               order_by = "recordId", 
  ...
  if (!is.null(data_sources)) {
    ...
    data <- list("formulas" = formulas, "dataSources" = data_sources,
                 "orderBy" = order_by, "orderDirection" = order_direction)
    
  } else {
    
    data <- list("formulas" = formulas, "orderBy" = order_by, 
